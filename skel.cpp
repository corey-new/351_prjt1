...
void computeHash(const string& hashProgName) {
		...
		cmdLine = cmdLine + " ";
		cmdLine = cmdLine + fileNameRecv;
		// cout << hashProgName << endl;
		// cout << cmdLine << endl;
	   /* TODO: Open the pipe to the program (specified in cmdLine)
		using popen() and save the ouput into hashValue. See popen.cpp for
		examples using popen. */
		FILE* fp = popen(cmdLine.c_str(), "r");
			/* Reset the value buffer */
			memset(hashValue, (char)NULL, HASH_VALUE_LENGTH);
				if(fread(hashValue,sizeof(char),sizeof(char)*HASH_VALUE_LENGTH,fp)
								 < 0) {
					perror("fread");
					exit(-1);
				}
